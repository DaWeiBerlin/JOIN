    ...
    for (let i = 0; i < array.length; i++) {
        ...
        if (!assigned.includes(userName)) {
            document.getElementById(`div${i}`).innerHTML += `
                <div id="d${i}" style="position: absolute; right: 1px;" onclick="assign2('${userName}', 'd${i}')">
...
function assign2(name,d){
    ...
    console.log(j)
    assigned.push(name)
    document.getElementById("assigned").innerHTML += /*html*/`<div id="e${assignvar}" class="assignFrame">${name} <img style="width: 15px;position: relative;left: 8px;top: 0px;" onclick="removeAssign('${name}', 'd${j}', 'e${assignvar}')" src="assets/img/close.png"></div>`
    document.getElementById(`d${j}`).style.display = "none"
    
    // }else{
    //     assigned.push(name)
    //     document.getElementById("assigned").innerHTML += /*html*/`<div id="e${assignvar}" class="assignFrame">${name} <img style="width: 15px;position: relative;left: 8px;top: 0px;" onclick="removeAssign('${name}', d, 'e${assignvar}')" src="assets/img/close.png"></div>`
    //     if(d){
...
function removeAssign(name, d , e){
    ...
    if(assignvar == 0){
        document.getElementById("assigned").innerHTML = "<div class='select'>Select Contacts to Assign</div>"
    }
    assign()
    ...
    if(closed == 1){
        ...
        document.getElementById("categories").innerHTML = /*html*/`
            <div id="cagtegoriesInner">
                <div class="category" onclick="addCategory('TechnicalTask');adjustCategory()" style="margin-top: 15px;margin-bottom: 10px">Technical Task</div>
                <div class="category" onclick="addCategory('UserStory');adjustCategory()" style="margin-bottom: 5px">User Story</div>
            </div>
        `;
        closed = 0
    } else{
...
function addsubtask(){
    if(document.getElementById("subtasksv").value != "" ){
        ...
        for(let i = 0; i < subtaskse2.length ; i++){
            document.getElementById("subtasks2").innerHTML +=/*html*/`
                <div class="sub" id="da${i}" ><div>${subtaskse2[i]["subtask"]} </div><div><img onclick="removeSubtask('${subtaskse2[i]['subtask']}');" src="assets/img/close.png" alt=""></div></div>`;
        }
    }else{return}
}
...
function removeSubtask(value){
    subtaskse2 = removeItem(subtaskse2, value)
    document.getElementById("subtasks2").innerHTML = ""
    if(subtaskse2 == undefined){
        subtaskse2 = []
        ...
        for(let i = 0; i < subtaskse2.length ; i++){
            document.getElementById("subtasks2").innerHTML +=/*html*/`
                <div class="sub" id="da${i}" ><div>${subtaskse2[i]} </div><div><img onclick="removeSubtask('da${i}')" src="assets/img/close.png" alt=""></div></div>
            `;
        }
    }
}
...
function clearb(){
    ...
    let yyyy = today.getFullYear();
    today = yyyy + "-" + mm + "-" + dd;
    document.getElementById("leftSide2").innerHTML = `
    ...
    document.getElementById("rightSide2").innerHTML = `
       <div style="display: flex;flex-direction: column;">
...
let height = 0
let adjust = false
function adjustAssign(){
    if(window.innerWidth < 900 && adjust == true){
        document.getElementById("rightSide2").classList.remove("important-top");
        height -= 100
    }
    if(window.innerWidth < 900 && adjust == false){
        document.getElementById("rightSide2").classList.add("important-top");
        height += 100
    }
    if(adjust == false){
        adjust = true
    ...
    document.getElementById("contentArea").style.height = `${1050 + height}px`
}

let adjust1 = false
function adjustCategory(){
    if(window.innerWidth < 900 && adjust1 == true){
        document.getElementById("tasks").classList.remove("important-top1");
        height -= 100
    }
    if(window.innerWidth < 900 && adjust1 == false){
        document.getElementById("tasks").classList.add("important-top1");
        height += 100
    }
    if(adjust1 == false){
        adjust1 = true
...
function searchTask(){
    let search = document.getElementById("textareaBoard").value.toLowerCase()
    let todos2 = []
    let todos3 = todos
    if(search == ""){
        todos2 = todos
    ...
    document.getElementById("modal").innerHTML = /*html*/`<div id="contentArea4">
            ...
            <div style="display: flex;justify-content: center;align-items: center;">
                <div id="leftSide" style="font-size: 16px;position: absolute;left: 160px;width: calc(50% - 160px);margin-bottom: 0px;top: 170px;">
                    ...
                    <div style="display: flex;flex-direction: column;font-size: 16px;">
                        <label for="">Assigned to</label>
                        <div onclick="assign()" id="assigned" value="Select contacts to assign" type="search" style="width: calc(100% - 60px);border-radius: 5px;border: 1px solid grey;margin-top: 5px;height: 32px;padding: 4px 30px 4px 4px;    background-color: white;font-size: 17px;z-index: 2;flex-wrap: wrap;overflow-x: auto;scrollbar-width: thin;"><div class="select">Select Contacts to Assign</div></div>
                    <div style="position: absolute;top: 294px;left: calc(100% - 70px);cursor: pointer;z-index: 2;" onclick="assign()"><img src="assets/img/category_menu.png" alt="" style="width: 18px;"></div>
                    <div id="contactse" style="display: none;width: calc(100% - 41px);border: 1px solid grey;border-bottom-left-radius: 5px;border-bottom-right-radius: 5px;max-height: 95px;position: relative;bottom: 10px;border-top: 1px solid transparent;padding: 9px 0 0 0;overflow-y: scroll;scrollbar-width: none;"></div>
                </div>
            </div>
                ...
                <div id="rightSide" style="position: absolute;width: calc(50% - 200px);left: calc(50% + 40px);top: 150px;">
                    <div style="display: flex;flex-direction: column;">
                        <label for="DueDate" style="margin-bottom: 5px;">Due Date <sup style="color: #ff8090;">*</sup></label>
                        <input id="duedate" type="date" value="${today}" style="border-radius: 5px;border: 1px solid grey;padding: 8px 6px;width: calc(100% - 50px);height: 24px;width: calc(100% - 13px);">
                    </div>
                    <div style="display: flex;flex-direction: column;margin-top: 17px;width: 100%;">
                        <label for="Prio" style="margin-bottom: 5px;">Prio</label>
                        <div id="Prio" style="display: flex;width: calc(100% - 50px);width: calc(100% + 0px);">
...
async function addTasks(){
    let title = document.getElementById("title3").value
    let description = document.getElementById("description").value
    let duedate = document.getElementById("duedate").value
    let accountUrl = `https://join-49eff-default-rtdb.europe-west1.firebasedatabase.app/tasks.json`;
    ...
    loadData2()
    document.getElementById("contentArea").innerHTML = `<div id="header3">Add Task</div>
        <div style="display: flex;justify-content: center;align-items: center;">
            <div id="leftSide2" style="font-size: 16px;position: absolute;left: 160px;width: calc(50% - 160px);margin-bottom: 0px;top: 201px;">
                <div style="display: flex;flex-direction: column;position: relative;bottom: 20px;">
                    <label for="Title">Title<sup style="color: #ff8090;">*</sup></label>
                    <textarea name="Title" id="title3" placeholder="Enter a title"></textarea>
                </div>
                <div style="display: flex;flex-direction: column;">
                    <label for="Description">Description</label>
                    <textarea name="Description" id="description" placeholder="Enter a Description"></textarea></div>
        ...
        <div id="required2"><sup style="color:#ff8090;">*</sup>This field is required</div>`;
    } else{
        alert("Biite fülle alle Felder bis auf subtasks aus und wähle eine Priorität aus.")
    }
}
...
function updateHTML() {
    ...
    document.getElementById('ToDo').innerHTML = '<div id="toDo"><div>To Do</div><div style="cursor: pointer;" onclick=addTaskse(`todo`)><img src="assets/img/plus.png"></div></div>'
    ...
    document.getElementById('InProgress').innerHTML = "<div id='inProgress'><div>In Progress</div><div style='cursor: pointer;' onclick=addTaskse(`InProgress`)><img src='assets/img/plus.png'></div></div>"
    ...
    document.getElementById('AwaitFeedback').innerHTML = "<div id='awaitFeedback'><div>Await Feedback</div><div style='cursor: pointer;' onclick=addTaskse(`AwaitFeedback`)><img src='assets/img/plus.png'></div></div>"
    ...
    document.getElementById('Done').innerHTML = "<div id='done'><div>Done</div><div style='cursor: pointer;' onclick=addTaskse(`Done`)><img src='assets/img/plus.png'></div></div>"
...
function generateTodoHTML(element) {
    ...
    if(element["subtasks"]){
        ...
        subtasks.push(done)
        subtasks.push(task)
        let a = /*html*/`<div style="cursor:pointer" onclick="openTask('${element['category']}','${element['title']}','${element['description']}','${element['duedate']}','${element['prio']}','${element['assigned']}','${subtasks}','${element['r']}')" draggable="true" ondragstart="startDragging(${element['id']})" class="todo">
